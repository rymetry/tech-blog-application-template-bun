...
Next.js 16 / React 19.2 / Tailwind CSS v4 で構築した、**Portfolio & Blog** 向けテンプレートです。microCMS をヘッドレス CMS として採用し、記事一覧・検索・タグ絞り込み・関連記事・前後記事ナビゲーションなどのブログ機能に加え、Projects / About / Contact を含むポートフォリオ導線を備えています。App Router ベースで SEO とパフォーマンス最適化（ISR、メタデータ、構造化データ、RSS など）を実装済みです。
...
- パフォーマンス
  - `'use cache'` + `cacheLife` / `cacheTag` によるデータキャッシュ（再検証間隔 300 秒）
...
- `bun run test:e2e` Playwright E2E テスト（CSP ポリシー検証 + 違反ゼロ確認）
...
- robots: `/robots.txt`
- 通常ページは `'use cache'` により 5 分間隔で再検証されます。
...
- CSP は proxy（`src/proxy.ts`）で HTML ナビゲーションにのみ適用します。`unsafe-inline` ベースのポリシーを採用し、nonce は使用しません。`script-src-attr 'none'` により inline event handler はブロックされます。
